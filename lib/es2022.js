...
import { buildFullPathSegment, get } from '@bablr/agast-helpers/path';
...
class ES2022Atrivial extends ES2021.atrivial {
  ...
  *ClassModifiers() {
    ...
    if (yield match(m`'{'`)) {
      freeze(modifiers);
      return r(null, freeze({ type: 'ClassStaticBlock', modifiers }));
    }
    let async_;
    ...
    freeze(modifiers);
    return r(null, freeze({ type, modifiers }));
  }
}

freeze(ES2022Atrivial);
freeze(ES2022Atrivial.prototype);
