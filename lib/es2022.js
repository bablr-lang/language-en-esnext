...
import ES2021, { startsIdentifierPattern } from './es2021.js';
import { parseObject, printSource } from '@bablr/agast-helpers/tree';
...
class ES2022Atrivial extends ES2021.atrivial {
  ...
  *ClassProperty({ props }) {
    ...
    if (yield eatMatch(m`openNameToken*: <* '[' />`)) {
      ...
      yield eatMatch(
        m`name+$: <*Identifier ${startsIdentifierPattern} />`,
        o({ scoped: false }),
        o({ bind: true }),
      );
    }
  ...
  *ClassModifiers() {
    ...
    } else if (yield eatMatch(m`<*Identifier ${startsIdentifierPattern} />`)) {
